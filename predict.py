...
def predict_cluster(user_data):
    ...
    X_tsne_orig = np.load(
        "X_tsne.npy"
    )  # Coordonnées TSNE pré-calculées du dataset d'origine
    orig_clusters = kmeans_optimal.labels_

    # Conversion des données utilisateur en DataFrame et prétraitement
    user_df = pd.DataFrame(
        [entry for entry in user_data if entry is not None and isinstance(entry, dict)]
    )
    features = [
        "acousticness",
        "danceability",
    ...
    user_features_pca_coords = (
        user_features_pca  # Si vous utilisez PCA comme approximation
    )
